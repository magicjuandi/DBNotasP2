...
public class GradeRepositoryimpl implements GradeRepository {
    ...
    public List<Grade> list(){
        ...
        try(Statement stat = getConnection().createStatement();
            ResultSet rs = stat.executeQuery("SELECT gra.id, gra.studentid, stu.name, stu.email, stu.semester, gra.subjectid, sub.name AS subject, sub.teacherid, tea.name AS teachername, tea.email AS teachermail, gra.grade FROM grade AS gra, student AS stu, subject AS sub, teacher AS tea WHERE gra.studentid = stu.id AND gra.subjectid = sub.id AND sub.teacherid = tea.id;")){
            while(rs.next()){
    ...
    public Grade byId(Long idStu, Long idSub){
        Grade grade = null;
        try(PreparedStatement preparedStatement = getConnection()
                .prepareStatement("SELECT gra.id, gra.studentid, stu.name, stu.email, stu.semester, gra.subjectid, sub.name AS subject, sub.teacherid, tea.name AS teachername, tea.email AS teachermail, gra.grade FROM grade AS gra, student AS stu, subject AS sub, teacher AS tea WHERE gra.studentid = stu.id AND stu.id = ? AND gra.subjectid = sub.id AND sub.id = ? AND sub.teacherid = tea.id;")){
            preparedStatement.setLong(1,idStu);
            preparedStatement.setLong(2,idSub);
